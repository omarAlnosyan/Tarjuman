...
import {
  User, BookOpen, Quote, Sparkles, Compass, History,
  Feather, Hash, Send, Menu, X, MessageSquare, Info, ArrowLeft, Trash2
} from 'lucide-react';
  ...
  try {
    ...
    if (!response.ok) {
      ...
      try {
        ...
        if (response.status === 404) errorMessage = 'الخادم غير متاح. شغّل الـ API: start_api.bat أو python -m uvicorn api.main:app --host 0.0.0.0 --port 8000';
        else if (response.status === 500) errorMessage = 'حدث خطأ في الخادم. يرجى المحاولة لاحقاً';
      }
    ...
    const text = errMsg?.includes('fetch') || errMsg?.includes('Failed to fetch')
      ? 'لا يمكن الاتصال بالخادم. شغّل الـ API من مجلد المشروع: start_api.bat أو نفّذ: python -m uvicorn api.main:app --host 0.0.0.0 --port 8000'
      : 'حدث خطأ في الاتصال. تأكد من تشغيل الـ API على http://localhost:8000';
    return { type: 'chat', text, error: text };
  }
};

const VerseCard = ({ data }: { data: SearchResult }) => (
  <div className="bg-white border border-[#d4af37]/20 rounded-2xl overflow-hidden shadow-xl animate-in">
...
export default function Home() {
      ...
      <div className="min-h-screen flex items-center justify-center" style={{ fontFamily: 'Amiri, serif', background: 'linear-gradient(135deg, #fdfbf7 0%, #f8f6f1 100%)' }}>
        <div className="max-w-4xl mx-auto px-6 py-12 text-center">
          <div className="mb-8 inline-block">
            <div
              className="transition-transform duration-700 hover:rotate-[360deg] cursor-default"
              style={{ width: '96px', height: '96px', background: 'linear-gradient(135deg, #d4af37 0%, #b8860b 100%)', borderRadius: '24px', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 20px 50px rgba(212, 175, 55, 0.3)', margin: '0 auto 1rem' }}
              title="ترجمان"
            >
  ...
  return (
    <div dir="rtl" className="h-screen flex flex-col font-sans" style={{ backgroundColor: COLORS.bg, color: COLORS.text }}>
      <div className="fixed inset-0 pointer-events-none opacity-[0.03]" style={{ backgroundImage: `url('https://www.transparenttextures.com/patterns/cream-paper.png')` }} />
      ...
      <header className="z-40 bg-white/80 backdrop-blur-md border-b border-[#d4af37]/10 px-6 py-4 flex items-center justify-between shadow-sm">
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-gradient-to-br from-[#d4af37] to-[#b8860b] rounded-xl flex items-center justify-center shadow-lg shadow-[#d4af37]/20 transition-transform duration-500 hover:rotate-[360deg] cursor-default" title="ترجمان">
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={clearChat}
            className="flex items-center gap-2 px-3 py-2 text-xs font-bold text-[#8b7355] hover:text-[#2c1810] hover:bg-[#f8f6f1] rounded-xl border border-[#d4af37]/10 transition-colors"
            title="مسح المحادثة"
          >
            <Trash2 size={16} />
          ...
          <div
            className="hidden md:flex items-center gap-1.5 text-[10px] px-3 py-1.5 rounded-full border transition-colors"
            style={{
              backgroundColor: apiConnected === true ? '#f0fdf4' : apiConnected === false ? '#fef2f2' : '#f8f6f1',
              color: apiConnected === true ? '#166534' : apiConnected === false ? '#991b1b' : '#8b7355',
              borderColor: apiConnected === true ? 'rgba(34,197,94,0.3)' : apiConnected === false ? 'rgba(239,68,68,0.3)' : 'rgba(212,175,55,0.2)'
            }}
            title={apiConnected === false ? 'شغّل الـ API: start_api.bat أو python -m uvicorn api.main:app --host 0.0.0.0 --port 8000' : undefined}
          >
            <div
              className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${apiConnected === null ? 'animate-pulse' : ''}`}
              style={{
                backgroundColor: apiConnected === true ? '#22c55e' : apiConnected === false ? '#ef4444' : '#94a3b8'
              }}
            />
            {apiConnected === true ? 'متصل بالدواوين' : apiConnected === false ? 'غير متصل — شغّل الـ API' : 'جاري التحقق...'}
          </div>
        </div>
      </header>
              ...
              {messages.map((msg) => (
                <div key={msg.id} className={`flex ${msg.role === 'user' ? 'justify-start' : 'justify-end'}`}>
                  <div className="max-w-[90%] md:max-w-[85%] space-y-3">
                    <div className={`p-5 rounded-2xl shadow-sm border leading-relaxed text-lg prose prose-sm max-w-none prose-p:my-1 prose-headings:my-2 ${
                      msg.role === 'user'
                ...
                <div className="flex justify-end">
                  <div className="bg-[#f8f6f1] p-4 rounded-2xl border border-[#d4af37]/10 flex items-center gap-3">
                    <div className="flex gap-1">
                      ...
                      <div className="w-1.5 h-1.5 bg-[#d4af37] rounded-full animate-bounce [animation-delay:-.3s]" />
                      <div className="w-1.5 h-1.5 bg-[#d4af37] rounded-full animate-bounce [animation-delay:-.5s]" />
                    </div>
